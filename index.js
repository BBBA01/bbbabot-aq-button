...
module.exports = BBBA = async (BBBA, mek) => {
  try {
...
const body = (type === 'listResponseMessage' && mek.message.listResponseMessage.title) ? mek.message.listResponseMessage.title : (type === 'buttonsResponseMessage' && mek.message.buttonsResponseMessage.selectedButtonId) ? mek.message.buttonsResponseMessage.selectedButtonId : (type === 'conversation' && mek.message.conversation.startsWith(prefix)) ? mek.message.conversation : (type == 'imageMessage') && mek.message.imageMessage.caption.startsWith(prefix) ? mek.message.imageMessage.caption : (type == 'videoMessage') && mek.message.videoMessage.caption.startsWith(prefix) ? mek.message.videoMessage.caption : (type == 'extendedTextMessage') && mek.message.extendedTextMessage.text.startsWith(prefix) ? mek.message.extendedTextMessage.text : (type == 'stickerMessage') && (getCmd(mek.message.stickerMessage.fileSha256.toString('base64')) !== null && getCmd(mek.message.stickerMessage.fileSha256.toString('base64')) !== undefined) ? getCmd(mek.message.stickerMessage.fileSha256.toString('base64')) : ""
    ...
    const botNumber = BBBA.user.jid;
    
    const botNumberss = BBBA.user.jid + "@c.us";
...
// 𝘼𝙪𝙩𝙤 𝙍𝙚𝙖𝙙 𝙂𝙘
var ampun = await BBBA.chats.array.filter(v => v.jid.endsWith('g.us'))
ampun.map( async ({ jid }) => {
if (readGc === false) return
await BBBA.chatRead(jid)
})
// 𝘼𝙪𝙩𝙤 𝙍𝙚𝙖𝙙 𝙋𝙘
var chatss = await BBBA.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
...
await BBBA.updatePresence(from, Presence.recording)
} else if (autoketik) {
	if (autoketik === false) return
await BBBA.updatePresence(from, Presence.composing)
}
...
const checkRegisteredUser = (sender) => {
    ...
    let senderr = mek.key.fromMe
      ...
      ? mek.participant
      : mek.key.remoteJid;
    const totalchat = await BBBA.chats.all();
    const ownerNumber = [`994407683218@s.whatsapp.net`]
	const senderNumber = sender.split("@")[0] 
    const m = simple.smsg(BBBA, mek);
    const groupMetadata = isGroup ? await BBBA.groupMetadata(from) : "";
   ...
   const isRegister = register.includes(sender)
    ...
    const conts = mek.key.fromMe
      ? BBBA.user.jid
      : BBBA.contacts[sender] || { notify: jid.replace(/@.+/, "") };
    ...
    const totalhit = JSON.parse(fs.readFileSync("./lib/totalcmd.json"))[0]
      .totalcmd;
const daftar1 = `👋𝗬𝗼𝗵𝗼𝗵𝗼❗${pushname} ${ucapanWaktu} \n\nBefore Using the Bot Verify First `
...
cmhit.push(command)
  ...
  /*const reply = (teks) => {
      BBBA.sendMessage(from, teks, text, { quoted: mek });
    };*/
    const textImg = (teks) => {
           return ikyy.sendMessage(from, teks, text, {quoted: freply, thumbnail: fs.readFileSync('./BBBA.jpg')})
        }
const math = (teks) => {
           return Math.floor(teks)
       }
/*const reply = (teks) => {
			BBBA.sendMessage(from, teks, text, { thumbnail: iye, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jam} - ${week} ${weton} - ${date}`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/994407683218?text=hello`}}})
		}*/
    const sendMess = (hehe, teks) => {
      BBBA.sendMessage(hehe, teks, text);
    };

    const mentions = (teks, memberr, id) => {
      id == null || id == undefined || id == false
        ? BBBA.sendMessage(from, teks.trim(), extendedText, {
...
const freply = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: '16504228206@s.whatsapp.net' } : {}) }, message: { "contactMessage": { "displayName": `${pushname}`, "vcard": `BEGIN:VCARD\nVERSION:3.0\nN:XL;${pushname},;;;\nFN:${pushname},\nitem1.TEL;waid=${senderr.split('@')[0]}:${senderr.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`, "jpegThumbnail":fs.readFileSync('./BBBA.jpg')
        }}}
...
const ftex = {
    ...
    const fakestatus = (teks) => {
      BBBA.sendMessage(from, teks, text, {
        quoted: {
          key: {
            fromMe: false,
            participant: `0@s.whatsapp.net`,
    ...
    const fakethumb = (teks, yes) => {
      BBBA.sendMessage(from, teks, image, {
...
BBBA.on('CB:action,,battery', json => {
...
download(url, "./stik" + names + ".png", async function () {
        ...
        exec(
          ...
          (err) => {
            ...
            BBBA.sendMessage(to, media, MessageType.sticker, { quoted: mek });
...
if (budy.toLowerCase() === `8473`){
		if (isRegister) return 
		    ...
		    BBBA.sendMessage(from, teks, text, {quoted: fkontak })
...
key: {
    ...
    const sendMediaURL = async (to, url, text = "", mids = []) => {
      ...
      download(url, filename, async function () {
        ...
        BBBA.sendMessage(to, media, type, {
          quoted: mek,
          mimetype: mime,
          caption: text,
...
BBBA.sendMessage(id, buttonMessage, MessageType.buttonsMessage, options)
}
///Button Image
const sendButImage = async(id, text1, desc1, gam1, but = [], options = {}) => {
kma = gam1
mhan = await BBBA.prepareMessage(from, kma, image)
...
BBBA.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
}
///Button Video
const sendButVideo = async(id, text1, desc1, vid1, but = [], options = {}) => {
kma = vid1
mhan = await BBBA.prepareMessage(from, kma, video)
const buttonMessages = {
...
kma = gam1
mhan = await BBBA.prepareMessage(from, kma, location)
const buttonMessages = {
...
BBBA.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
    ...
    const kick = function (from, orangnya) {
      for (let i of orangnya) {
        BBBA.groupRemove(from, [i]);
      }
    };
    const add = function (from, orangnya) {
      BBBA.groupAdd(from, orangnya);
    };
        const grupinv = (teks) => {
        	grup = BBBA.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Dgt6JhzTvlmEor8Zz23fHx', "groupName": `BBBA Bot Inc.`, "footerText": "*_𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021_*", "jpegThumbnail": iye, "caption": teks}}, {quoted:mek})
            BBBA.relayWAMessage(grup)
        }
const fakeitem = (teks) => {
            BBBA.sendMessage(from, teks, text, {
                quoted: {
        ...
        participant:`0@s.whatsapp.net`, ...(from ? {
remoteJid :"6289523258649-1604595598@g.us" }: {})
                    },message:{"orderMessage":{"orderId":"174238614569481","thumbnail":fs.readFileSync(`./stik/thumb.jpeg`),"itemCount":10,"status":"INQUIRY","surface":"CATALOG","message":`𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021`,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}}, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true})}
// Katalog
const reply = (teks) => {
             res = BBBA.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 111119999, "message": teks, "footerText": "*BBBA*", "thumbnail": bgg, "surface": 'CATALOG' }}, {quoted:ftrol})
             BBBA.relayWAMessage(res)
        }
const fakewa = (teks) => {        
BBBA.sendMessage(from, teks, text, { thumbnail: fake, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `BBBA`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/994407683218?text=hello`}}})
		}
const replyfakelink = (teks) => {
BBBA.sendMessage(from, teks, text,{contextInfo :{text: 'hi',
"forwardingScore": 1000000000,
isForwarded: false,
sendEphemeral: false,
"externalAdReply": {
                "title": `Whatsapp-BOT\n𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021. (@nnda.krnwn_) •Instagram photos and Videos`,
                "body": "",
                "previewType": "PHOTO",
...
"externalAdReply": {
                "title": `Whatsapp`,
                "body": `wa.me/994407683218`,
                "mediaType": "10",
                "mediaUrl": `wa.me/994407683218`,
                "thumbnailUrl": "https://i.ibb.co/y0RYgzB/FB-IMG-1635413002830.jpg",
                "thumbnail": iye,
                "sourceUrl": `wa.me/994407683218`,
...
// Support Di Versi 3.5.2 
    const sendBug = async (target, teks) => {
      if (!teks) teks = ".";
      await BBBA.relayWAMessage(
        BBBA.prepareMessageFromContent(
          target,
          BBBA.prepareDisappearingMessageSettingContent(0),
          {}
        ),
        { waitForAck: true }
      );
      BBBA.sendMessage(target, teks, "conversation");
    };
...
async function sendFileFromUrl(from, url, caption, msg, men) {
            ...
            return BBBA.sendMessage(from, await getBuffer(url), type, {caption: caption, quoted: mek, mimetype: mime, contextInfo: {"mentionedJid": men ? men : []}})
        }
    ...
    if (isGroup && !isVote) {
      if (budy.toLowerCase() === "vote") {
        ...
        let id_vote = sender ? sender : "994407683218@s.whatsapp.net";
...
await BBBA.setStatus(`Bot Mode On Since ${uptime}`).catch((_) => _);
...
const serialUser = createSerial(18)
	         try {
								ppimg = await BBBA.getProfilePicture(`${sender.split('@')[0]}@c.us`)
								} catch {
...
*Thank you for registering in the BBBA Bot Database🙇🏻‍♀️*`
         haitod = await getBuffer(`http://hadi-api.herokuapp.com/api/card/verify?nama=${encodeURI(pushname)}&member=${_registered.length}&seri=${serialUser}&pp=${ppimg}&bg=${ppimg}`)
             buttons = [{buttonId: `menu`,buttonText:{displayText: `𝗠𝗘𝗡𝗨🎾`},type:1}]
              imageMsg = (await BBBA.prepareMessageMedia(haitod, "imageMessage", { thumbnail: haitod, })).imageMessage
              buttonsMessage = {footerText:'\n\n𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021', imageMessage: imageMsg,
              contentText:`${anuu}`,buttons,headerType:4}
              prep = await BBBA.prepareMessageFromContent(from,{buttonsMessage},{quoted: fkontak})
              BBBA.relayWAMessage(prep)
	         console.log(color('[REGISTER]'), color(time, 'yellow'), 'Serial:', color(serialUser, 'cyan'), 'in', color(sender || groupName))
	    // console.log(e)
            setTimeout( () => {
			BBBA.updatePresence(from, Presence.composing)
...
case 'help':
case 'menu':
groups = BBBA.chats.array.filter(v => v.jid.endsWith('g.us'))
              privat = BBBA.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
              ram2 = `${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB`
           
              uptime = process.uptime();
              timestampe = speed();
              totalChat = await BBBA.chats.all()
              latensie = speed() - timestampe
              total = math(`${groups.length}*${privat.length}`)
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
       txtt =`👋𝗬𝗼𝗵𝗼𝗵𝗼❗${pushname}
       
𝗚𝗿𝗼𝘂𝗽 𝗖𝗵𝗮𝘁𝘀 : ${groups.length} 
𝗣𝗿𝗶𝘃𝗮𝘁𝗲 𝗖𝗵𝗮𝘁𝘀 : ${privat.length}
𝗧𝗼𝘁𝗮𝗹 𝗖𝗵𝗮𝘁𝘀 : ${totalChat.length}
𝗦𝗽𝗲𝗲𝗱 : ${latensie.toFixed(4)} Second
𝗔𝗰𝘁𝗶𝘃𝗲 : ${runtime(process.uptime())}
𝗣𝗹𝗮𝘁𝗳𝗼𝗿𝗺 : ${os.platform()}

*Note:* If menu doesn't appear on clicking menu button then, TYPE #allmenu 📇`
               buttons = [{buttonId:`allmenu`, 
               buttonText:{displayText: '𝗠𝗘𝗡𝗨🎾'},type:1},
               {buttonId:`owner`,buttonText:{displayText:'𝗖𝗥𝗘𝗔𝗧𝗢𝗥👒'},type:1},
               {buttonId:`command`,buttonText:{displayText:'𝗟𝗜𝗦𝗧 𝗠𝗘𝗡𝗨📝'},type:1}]


               imageMsg = (await BBBA.prepareMessageMedia(fs.readFileSync(`./stik/oksip.jpeg`), 'imageMessage', {thumbnail: fs.readFileSync(`./stik/oksip.jpeg`)})).imageMessage
...
groups = BBBA.chats.array.filter(v => v.jid.endsWith('g.us'))
              ...
              total = math(`${groups.length}*${privat.length}`)
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
        ptod = "994407683218@s.whatsapp.net"
        dtod = '994407683218'
        stod = `${sender}`
        uwuu = '```'
        yoii = '*'
       stst = await BBBA.getStatus(`${sender.split('@')[0]}@c.us`)
				stst = stst.status == 401 ? '' : stst.status
			num = await fetchJson(`https://api.telnyx.com/anonymous/v2/number_lookup/${senderNumber}`, {method: 'get'})
       menu = `
 👋𝗬𝗼𝗵𝗼𝗵𝗼❗@${sender.split("@")[0]} it's me 👒𝐁 𝐁 𝐁 𝐀 𝐁☠𝐓© thanks for joining our alliance me and my owner will protect your group as per our contract so let us start our endless journey together because this group will be our sailing ship where we will stand by you just like that I know you'll stand by us, together we can conquer the whole world and find everything that we are looking for~~⚓
${readmore}
...
*Note:* Press the button to see 
the listed options follow their usage is given side by side 📇
`
sendButLocation(from, `${menu}`,`𝗕𝗼𝘁 𝗕𝘆 @${dtod.split("@")[0]}`, {jpegThumbnail:iye}, [{buttonId:`command`,buttonText:{displayText:'𝗟𝗜𝗦𝗧 𝗠𝗘𝗡𝗨📝'},type:1},{buttonId:`credit`,buttonText:{displayText:'𝗖𝗥𝗘𝗗𝗜𝗧𝗦🤝'},type:1},{buttonId:`script`,buttonText:{displayText:'𝗦𝗖𝗥𝗜𝗣𝗧📂'},type:1}], {contextInfo: { mentionedJid: [ptod,stod]}})
...
case 'credit':
 ...
 🌾 : wa.me/${dtod2.split('@')[0]}
 `
     credit = BBBA.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 1000, "message": `${yez}`, "footerText": "hehe", "thumbnail": iye, "surface": 'CATALOG'}}, {quoted: mek})
            BBBA.relayWAMessage(credit)
        break
case 'suit':
...
case 'bugreport':
              ...
              BBBA.sendMessage('994407683218@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
                ...
                BBBA.sendMessage(from, dapuhy, image, {quoted: mek})        
                break    
case 'auu': // by BBBA
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
       reply('No Sis')
...
break
case 'h':
reply('*So long!....he/she is gone it was fun while it lasted🎐*')
break
case 'okeyx':
reply('*AYY-AYY! WELCOME ABOARD! LET HAVE A TOAST FOR OUR NEW CREWMATE🍻*')
break
...
case 'iggw':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
  reply('https://www.instagram.com/josephBBBA13/')
break
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
               ...
               await sleep(1000)
               haibg =`*Yohoho!* ${pushname}🐟\nYepp thats my owner and our ships captain🦑, What do you want to know about??`
               buttons = [{buttonId: `mygithub`,buttonText:{displayText: '𝗦𝗖 𝗕𝗢𝗧'},type:1},{buttonId:`iggw`,buttonText:{displayText:'𝗜𝗡𝗦𝗧𝗔𝗚𝗥𝗔𝗠'},type:1}]
               buttonsMessage = { contentText: `${haibg}`, footerText: `\n\nCreated By *${fakeyoi}* 🐚`, buttons: buttons, headerType: 1 }
               prep = await BBBA.prepareMessageFromContent(from,{buttonsMessage},{})
               BBBA.relayWAMessage(prep)
               break     
...
BBBA.sendMessage(from, { "degreesLatitude": '', "degreesLongitude": '', "jpegThumbnail": iye}, MessageType.location, {quoted: ikymemek})
           break
case 'mengyoi':
if (!mek.key.fromMe) return;
sendBug(from)
...
case 'buglink':
                if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
                 BBBA.toggleDisappearingMessages(from, 0)
               BBBA.sendMessage(from, virtex3(prefix), text, { quoted:ftrol, contextInfo :{text: '🔥',
            "forwardingScore": 1000000000,
            isForwarded: false,
            sendEphemeral: false,
            "externalAdReply": {
                ...
                "previewType": "PHOTO",
                "thumbnailUrl": "https://i.ibb.co/y0RYgzB/FB-IMG-1635413002830.jpg",
                "thumbnail": fs.readFileSync(`./BBBA.jpg`),
...
BBBA.sendMessage(from, listMsg, MessageType.listMessage, {quoted:ftrol})
BBBA.toggleDisappearingMessages(from, 0)
break
case 'bugcatalog':
if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
BBBA.toggleDisappearingMessages(from, 0)
 babi = (teks) => {
             res = BBBA.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 9999999, "message": teks, "footerText": `${emoji2(prefix)}`, "thumbnail": iye, "surface": 'CATALOG' }}, {quoted: {
  key: {
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
                                    if (error) return reply(mess.error.api)
									 BBBA.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: freply})
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
		...
		damdu = fs.readFileSync(`./sticker/${random}.webp`)
			BBBA.sendMessage(from, damdu, sticker, {quoted: mek})
			break
 case 'owner3':
let ini_list = []
for (let i of nomorowner1) {
const vname = BBBA.contacts[i] != undefined ? BBBA.contacts[i].vname || BBBA.contacts[i].notify : undefined
ini_list.push({
"displayName": 'BBBA',
"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:${vname ? `${vname}` : `${BBBA.user.name}`}\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
})
}
hehe = await BBBA.sendMessage(from, {
"displayName": `${ini_list.length} contact`,
"contacts": ini_list 
}, 'contactsArrayMessage', { quoted: mek })
BBBA.sendMessage(from,'This is my owners contact',text,{quoted: mek})
break
case 'command':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
 listMsg = {
 buttonText: '𝗟𝗜𝗦𝗧 𝗠𝗘𝗡𝗨📝',
 footerText: '\n\n𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021',
 description: `*Aye-Aye!* @${sender.split('@')[0]}, bottoms up 🍻 sailor click and select the menus of your choice here. 🌊`,
 sections: [
                     {
                      "title": `BBBA`,
 rows: [
...
BBBA.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [sender]},quoted:ftrol})
break
case 'tagmenu':
menu = `━━❰ •𝙏𝙖𝙜🏷️𝙈𝙚𝙣𝙪• ❱━━
...
㋛${prefix}totag
`
sendButMessage(from, menu, `𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021`, [
          {
            buttonId: `command`,
            buttonText: {
              displayText: `𝗕𝗔𝗖𝗞☸𝗠𝗘𝗡𝗨 `,
            },
            type: 1,
          },]);
break
...
㋛${prefix}herodetail <heroname>

㋛${prefix}herolist <heroname>
...
㋛${prefix}getbio <@tag member>

㋛${prefix}getname <@tag member>
...
break
case 'catalog':
      list = await BBBA.prepareMessageFromContent(from, {
    "listMessage": {
      "title": "PAYMENT",
      "description": "CLICK",
      "buttonText": "",
      ...
      "footerText": " 𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021"
    }
  }, {quoted: freply})
  BBBA.relayWAMessage(list, {waitForAck: true})
       
case 'triggered':
                
                if (isImage || isQuotedImage) {
                    let encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
                    let media = await BBBA.downloadAndSaveMediaMessage(encmedia)
                ...
                exec(`webpmux -set exif ./sticker/data.exif ./sticker/triggered.webp -o ./sticker/triggered.webp`, async (error) => {
                    if (error) return reply(mess.error.api)
                    BBBA.sendMessage(from, fs.readFileSync(`./sticker/triggered.webp`), sticker, {quoted: freply})
...
case 'script':
		case 'sc':
		case 'sourcecode':
		BBBA.sendMessage(from, { text: "https://github.com/DGBBBA/DogeBot", matchedText: 'nopee', description: "", title: "What are you doing?", jpegThumbnail: iye }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
      //------------------< Sticker Cmd >-------------------
      case "addcmd":
      case "setcmd":
...
if (args.length < 1) return reply('The text?')
                    teks = body.slice(10)
                    BBBA.sendMessage('status@broadcast', teks, MessageType.text)
...
if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
  if (args.length < 1) return reply('The text?')
                    teks = body.slice(12)
                    BBBA.sendMessage('status@broadcast', {degreesLatitude: 24.121231, degreesLongitude: 55.1121221, name:teks,address:`${fakeyoi}`}, MessageType.location)
...
if (isMedia && !mek.message.videoMessage || isQuotedSticker) {
						...
						buff = await BBBA.downloadMediaMessage(encmedia)
						BBBA.sendMessage('status@broadcast', buff, sticker)
						}
...
if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						...
						buff = await BBBA.downloadMediaMessage(encmedia)
						BBBA.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 359996400})
						}
...
if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						const encmedia = isQuotedAudio ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await BBBA.downloadMediaMessage(encmedia)
						BBBA.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt: true})
						}
...
if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					...
					var mediap = await BBBA.downloadAndSaveMediaMessage(enmediap)
                    const buffer3 = fs.readFileSync(mediap)
                    BBBA.sendMessage('status@broadcast', buffer3, MessageType.video, {duration: 359996400, caption: `${konti}`})
...
if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					...
					media = await BBBA.downloadAndSaveMediaMessage(enmedia)
                    const buffer6 = fs.readFileSync(media)
                    BBBA.sendMessage('status@broadcast', buffer6, MessageType.video, {mimetype : 'video/gif', caption: `${konti}`})
                    ...
                    BBBA.sendMessage('status@broadcast', buffer, MessageType.image, {quoted: mek, caption: `${teksyy}`})
                    reply(`Image upload success:\n${teksyy}`)
                        break
					case 'shutdown':
					if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
				return BBBA.sendMessage(from, JSON.stringify(eval(process.exit())))
				reply('Okey')
				break
      case "delcmd":
        if (!isQuotedSticker)
...
for(let i of bancht){
met = await BBBA.groupMetadata(i)
teks += 'Id : ' + i + '\n'
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
                  sange = body.slice(1)
					...
					BBBA.sendMessage(from, 'Question : *'+sange+'*\n\nAnswer: '+ nge+'%', text, { quoted: mek })
					break
                case 'gaycheck':
					// source code by BBBA⛔
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
                  gayy = body.slice(1)
					...
					BBBA.sendMessage(from, 'Question : *'+gayy+'*\n\nAnswer : '+ yag+'%', text, { quoted: mek })
					break
                case 'lesbicheck':
					// source code by BBBA⛔
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
                  lesbii = body.slice(1)
					...
					BBBA.sendMessage(from, 'Question : *'+lesbii+'*\n\nAnswer : '+ bi+'%', text, { quoted: mek })
					break
                case 'handsomecheck':
					// source code by BBBA⛔
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
ganteng = body.slice(1)
					...
					BBBA.sendMessage(from, 'Question : *'+ganteng+'*\n\nAnswer : '+ teng+'%', text, { quoted: mek })
					break
		case 'beautycheck':
					// source code by BBBA⛔
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
cantik = body.slice(1)
					...
					BBBA.sendMessage(from, 'Question : *'+cantik+'*\n\nAnswer : '+ tik+'%', text, { quoted: mek })
					break
		case 'character':
					// source code by BBBA⛔
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
watak = body.slice(1)
					...
					BBBA.sendMessage(from, 'Question : *'+watak+'*\n\nAnswer : '+ tak, text, { quoted: mek })
				        break
                case 'can':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					...
					BBBA.sendMessage(from, 'Question : *'+bisakah+'*\n\nAnswer : '+ keh, text, { quoted: mek })
					break
				case 'when':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					...
					BBBA.sendMessage(from, 'Question : *'+kapankah+'*\n\nAnswer : '+ koh, text, { quoted: mek })
					break
         			  case 'is':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					apakah = body.slice(1)
					...
					BBBA.sendMessage(from, 'Question : *'+apakah+'*\n\nAnswer : '+ kah, text, { quoted: mek })
					break
				case 'rate':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					rate = body.slice(1)
					...
					BBBA.sendMessage(from, 'Question : *'+rate+'*\n\nAnswer : '+ te+'%', text, { quoted: mek })
					break
                  ...
                  BBBA.sendMessage(from, anu, image, {thumbnail: Buffer.alloc(0),caption: `OK it's done\n\nhow is it?`, quoted: mek})
                   break
                           case 'wolf':
                   if (args.length < 1) return reply(`[  ×  ] Example :\n*${prefix}${command} BBBA*`)
                   F = body.slice(6)
      ...
      case "antihidetag":
        ...
        if (!isGroupAdmins && !mek.key.fromMe) return reply("admin only");
        if (args[0] == "on") {
          if (isAntihidetag) return reply("*ACTIVE*");
          antihidetg.push(from);
          fs.writeFileSync(
            "./database/antihidetag.json",
            JSON.stringify(antihidetg)
          );
          reply("antihidetag turned *ON*");
        } else if (args[0] == "off") {
      ...
      case "antiviewonce":
        ...
        if (args[0] == "on") {
          ...
          reply("antiviewonce turned *ON*");
        } else if (args[0] == "off") {
          antivo.splice(from, 1);
          fs.writeFileSync("./database/antivo.json", JSON.stringify(antivo));
          reply("antiviewonce turned *OFF*");
        } else if (!q) {
          sendButMessage(
            from,
            `MODE ANTIVIEWONCE`,
...
autoketik = true
reply("autotype turned *ON*")
} else if (args[0] === "off") {
if (autoketik === false) return
autoketik = false
reply("autotype turned *OFF*")
} else {
...
autovn = true
reply("autovn turned *ON*")
} else if (args[0] === "off") {
if (autovn === false) return
autovn = false
reply("autovn turned *OFF*")
} else {
reply("Choose ON or OFF")
}
break
...
break
      case "autojoin":
        ...
        if (args[0] == "on") {
          ...
          autojoin = true;
          reply("autojoin turned *ON*");
        } else if (args[0] == "off") {
          autojoin = false;
          reply("autojoin turned *OFF*");
        } else if (!q) {
      ...
      case "antivirtex":
        ...
        if (args[0] == "on") {
          ...
          reply("antivirtex turned *OFF*");
        } else if (args[0] == "off") {
          antivirtex.splice(from, 1);
          fs.writeFileSync("./database/antivirtex.json", JSON.stringify(ant));
          reply("antivirtex turned *ON*");
        } else if (!q) {
      ...
      case "kickarea":
        ...
        if (!isGroupAdmins && !mek.key.fromMe) return reply("admin only");
        if (args[0] == "on") {
          if (isKickarea) return reply("*ACTIVE*");
          kickarea.push(from);
          fs.writeFileSync(
            "./database/antibule.json",
            JSON.stringify(kickarea)
          );
          reply("kickarea turned *ON*");
        } else if (args[0] == "off") {
          ...
          reply("kickarea turned *OFF*");
        } else if (!q) {
          sendButMessage(from, `*MODE KICKAREA*`, `\n𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021`, [
            {
...
if (!isGroup) return reply(`*Only group*`)
             ...
             let online = [...Object.keys(BBBA.chats.get(ido).presences), BBBA.user.jid]
             BBBA.sendMessage(from, '*Online list:*\n' + online.map(v => '👒@' + v.replace(/@.+/, '')).join `\n`, text, { quoted: mek, contextInfo: { mentionedJid: online }})
             } catch (e) {
             reply(`${e}`)
}
             break
      case "sider":
        ...
        let members = m.quoted.chat.endsWith("g.us")
          ? (await BBBA.groupMetadata(m.quoted.chat)).participants.length - 1
          : m.quoted.chat.endsWith("@broadcast")
          ? -1
          : 1;
        let { reads, deliveries } = await BBBA.messageInfo(
          m.quoted.chat,
          m.quoted.id
        );
        let txt = `
...
`.trim();
        m.reply(txt, null, {
          contextInfo: {
            mentionedJid: BBBA.parseMention(txt),
          },
        });
        break;
      case "q":
        if (!m.quoted) return reply("reply message!");
        let qse = BBBA.serializeM(await m.getQuotedObj());
        if (!qse.quoted)
      ...
      case "kick":
        ...
        if (mentioned.length > 1) {
          BBBA.groupRemove(from, mentioned);
          reply(mess.success);
        } else if (mentioned.length < 1) {
          anu = mek.message.extendedTextMessage.contextInfo.participant;
          BBBA.groupRemove(from, [anu]);
          reply(mess.success);
        } else {
          BBBA.groupRemove(from, mentioned);
          reply(mess.success);
...
case 'igstory': 
            ...
            for(let i of result.medias){
                if(i.url.includes('mp4')){
                    ...
                    BBBA.sendMessage(from,link,video,{quoted: mek,caption: `Type : ${i.type}`})
                } else {
                    let link = await getBuffer(i.url)
                    BBBA.sendMessage(from,link,image,{quoted: mek,caption: `Type : ${i.type}`})                  
                }
            }
            });
            break
case 'linkwa':
      ...
      case "getbio":
        ...
        var p = await BBBA.getStatus(`${yy}`, MessageType.text);
        reply(p.status);
        if (p.status == 401) {
        ...
        case 'getdeskgc':
              if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
				if (!isGroup) return reply('this feature is only for groups')
					anu = from
			   metadete = await BBBA.groupMetadata(anu)
				BBBA.sendMessage(from, metadete.desc, text, {quoted:mek})
				  break
      // Get Name
      case "getname":
        var ambl = mek.message.extendedTextMessage.contextInfo.participant;
        const sname =
          BBBA.contacts[ambl] != undefined
            ? BBBA.contacts[ambl].sname || BBBA.contacts[ambl].notify
            : undefined;
        reply(sname);
        break;
      case "setdesc":
        ...
        if (!isBotGroupAdmins) return reply("Bot not admin");
        if (!isGroup) return;
        BBBA.groupUpdateDescription(from, `${args.join(" ")}`);
        BBBA.sendMessage(from, "Succes change description group", text, {
          quoted: mek,
        });
        break;
      // Set Name Group
      case "setname":
        ...
        if (!isBotGroupAdmins) return reply("Bot not admin");
        if (!isGroup) return;
        BBBA.groupUpdateSubject(from, `${args.join(" ")}`);
        BBBA.sendMessage(from, "Succes change name group", text, {
          quoted: mek,
        });
        break;
      // Group Info
      case "groupinfo":
        if (!isGroup) return;
        ppUrl = await BBBA.getProfilePicture(from); // leave empty to get your own
        buffergbl = await getBuffer(ppUrl);
        BBBA.sendMessage(from, buffergbl, image, {
          quoted: mek,
      ...
      case "demote":
        ...
        BBBA.groupDemoteAdmin(from, [mentionede]);
      ...
      case "promote":
        ...
        mentionede = mek.message.extendedTextMessage.contextInfo.participant;
        BBBA.groupMakeAdmin(from, [mentionede]);
      ...
      case "closegc":
        ...
        BBBA.groupSettingChange(from, GroupSettingChange.messageSend, true);
        break;
case 'tospam':
...
if (!Number(oi2)) return reply('Amount must be a number!')
	  for (let i = 0; i < oi2; i++) {
	  BBBA.sendMessage(from, `${oi1}`, MessageType.text)
	  }
...
if (Number(args[0]) >= 50) return reply('Most!')
	  ...
	  BBBA.sendMessage(from, teks, MessageType.text)
	  }
} else if (isQuotedSticker) {
	encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	         median = await BBBA.downloadAndSaveMediaMessage(encmedian)
				anu = fs.readFileSync(median)
	if (!Number(args[0])) return reply('Amount must be a number!')
	if (Number(args[0]) >= 50) return reply('Most!')
	  for (let i = 0; i < args[0]; i++) {
	  BBBA.sendMessage(from, anu, sticker)
	  }
} else if (isQuotedAudio) {
	encmediat = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	            mediat = await BBBA.downloadAndSaveMediaMessage(encmediat)
				anu = fs.readFileSync(mediat)
	if (!Number(args[0])) return reply('Amount must be a number!')
	if (Number(args[0]) >= 50) return reply('Most!')
	  for (let i = 0; i < args[0]; i++) {
	  BBBA.sendMessage(from, anu, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true})
	  }
} else if (isQuotedImage) {
	boij = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
	delb = await BBBA.downloadMediaMessage(boij)
	teks = body.slice(6)
	oi1 = teks.split('|')[0]
oi2 = teks.split('|')[1]
if (Number(oi2) >= 50) return reply('Most!')
	if (!Number(oi2)) return reply('Amount must be a number!')
	  for (let i = 0; i < oi2; i++) {
	  BBBA.sendMessage(from, delb, MessageType.image, {caption: oi1})
	  }
}
	  break
      case "revoke":
      ...
      case "opengc":
        ...
        BBBA.groupSettingChange(from, GroupSettingChange.messageSend, false);
        break;
      case "reminder": // by Slavyan
        ...
        if (!isQuotedImage && !isQuotedSticker && !isQuotedAudio) {
          await BBBA.sendMessage(
            from,
            `REMINDER *ON*

*💬Message:* ${messRemind}
*🔰Type:* Text
*⏰Duration:* ${parsedTime.hours} hrs ${parsedTime.minutes} min ${
              parsedTime.seconds
            } sec
*📛To:* @${sender.split("@")[0]}
    `,
...
*🔰Type:* ${reminder.getReminderType(sender, _reminder)}`,
          ...
          media = await BBBA.downloadAndSaveMediaMessage(encmedia);
          await BBBA.sendMessage(
            from,
            `REMINDER *ON*
...
*🔰Type:* ${reminder.getReminderType(sender, _reminder)}`,
              ...
              BBBA.sendMessage(from, fs.readFileSync(media), sticker);
              _reminder.splice(
                reminder.getReminderPosition(sender, _reminder),
                1
              );
...
*💬Message:* ${messRemind}
*🔰Type:* ${reminder.getReminderType(sender, _reminder)}`;
              BBBA.sendMessage(from, media, image, {
                contextInfo: { mentionedJid: [sender] },
                caption: teks,
              });
              _reminder.splice(
...
*🔰Type:* ${reminder.getReminderType(sender, _reminder)}`,
              ...
              BBBA.sendMessage(from, fs.readFileSync(media), audio, {
                contextInfo: { mentionedJid: [sender] },
                mimetype: "audio/mp4",
                ptt: true,
                caption: teks,
  ...
  if (!mek.key.fromMe) return;
              ...
              reply('Otw Hack')
                tessgc = await getBuffer(`https://i.ibb.co/m4Qx3JG/20210319-204838.jpg`)
                   BBBA.updateProfilePicture (from, tessgc)
                   await sleep(1000)
                BBBA.groupUpdateSubject(from, `HACKED BY ${body.slice(8)}`)
                await sleep(1000)
                BBBA.groupUpdateDescription(from, `_${pushname} hacked this group_`)             
                await sleep(1000)
                BBBA.sendMessage(from, 'Succes Hacked', text, {quoted: fyt})
case 'mygithub':
  reply('https://github.com/DGBBBA')
					break;
   case 'ss':
      reply(mess.wait)
   ...
   sendButVideo(from, kodo, `Click Next To Continue`, get_result, [                      
          {
            ...
            type: 1,
                    ...
                    break    
case '+91':
                    get_result = await getBuffer(`https://BBBA.herokuapp.com/api/asupan?apikey=${valkey}`)
                    pll = `Intake`
...
filsize = args[0]
costick = await BBBA.prepareMessageFromContent(from,{
"stickerMessage": {
"url": m.quoted.url,
...
BBBA.relayWAMessage(costick)
break

case 'sizeimg':
if (args.length < 1) return reply('Enter the numbers')
filsizei = args[0]
costick3 = await BBBA.prepareMessageFromContent(from,{
"imageMessage": {
	"url": m.quoted.url,
	"mimetype": m.quoted.mimetype,
...
*Model* : ${i.phone.device_model}\n\n`;
	...
	case 'chat':
            ...
            var org = pc.split("|")[1];
            BBBA.sendMessage(nomor+'@s.whatsapp.net', org, MessageType.text)   
            reply(`Sukses mengirim chat:\n${org},@${nomor}`)
            break
case 'viewonce':
res = await BBBA.prepareMessageFromContent(from,{
"viewOnceMessage": {
"message": {
"imageMessage": {
"mimetype": 'image/jpeg',
    ...
    "hydratedButtons": [
      {
        "urlButton": {
          ...
          "url": "https://github.com/DGBBBA/DogeBot"
        },
        "index": 0
      }
    ]
      ...
      case "instagram":
        ...
        hx.igdl(args[0]).then(async (result) => {
          for (let i of result.medias) {
            if (i.url.includes("mp4")) {
              let link = await getBuffer(i.url);
              BBBA.sendMessage(from, link, video, {
                quoted: mek,
                caption: `Type : ${i.type}`,
              });
            } else {
              let link = await getBuffer(i.url);
              BBBA.sendMessage(from, link, image, {
                quoted: mek,
                caption: `Type : ${i.type}`,
              });
            }
...
case 'setprefix':
    ...
    Follower: ${tod.result.user_followers}`;
        BBBA.sendMessage(from, buff, image, { quoted: mek, caption: tt });
        break;*/
    ...
    Follower: ${i.result.stats.followerCount}`;
        BBBA.sendMessage(from, buff, image, { quoted: mek, caption: ii });
        break;
   
  case 'writebook':            
            if (args.length == 0) return reply(`example: ${prefix + command} BBBA`)
            reply(mess.wait)
            teks = args.join(" ")
            ini_buf = await getBuffer(`https://BBBA.herokuapp.com/api/maker/nulis?apikey=${valkey}&text=${teks}`)  
buttons = [{buttonId: `writelist`,buttonText:{displayText: `Write List🔖`},type:1}]
              imageMsg = (await BBBA.prepareMessageMedia(ini_buf, "imageMessage", { thumbnail: ini_buf, })).imageMessage
              buttonsMessage = {footerText:'𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await BBBA.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              BBBA.relayWAMessage(prep)
...
sendButMessage(from, tiyo, `𝐁 𝐁 𝐁 𝐀 𝐁☠??`, [
            {              
   ...
   case 'bc':
      case 'broadcast':
        if (!mek.key.fromMe) return;
             ...
             anu = await BBBA.chats.all()
             if (isMedia && !mek.message.videoMessage || isQuotedImage) {
             const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
             bc = await BBBA.downloadMediaMessage(encmedia)
             for (let _ of anu) {
             BBBA.sendMessage(_.jid, bc, image, {quoted:fkontak,caption: `*「 𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021 BROADCASTING 」*\n\n${body.slice(4)}`})
}
             reply('Broadcast success')
             } else {
             for (let _ of anu) {
BBBA.sendMessage(_.jid, 
...
buttons = [{buttonId: `yuri`,buttonText:{displayText: `➡️𝗡𝗘𝗫𝗧`},type:1}]
              imageMsg = (await BBBA.prepareMessageMedia(kon, "imageMessage", { thumbnail: kon, })).imageMessage
              buttonsMessage = {footerText:'𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await BBBA.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              BBBA.relayWAMessage(prep)
...
buttons = [{buttonId: `anal`,buttonText:{displayText: `➡️𝗡𝗘𝗫𝗧`},type:1}]
              imageMsg = (await BBBA.prepareMessageMedia(aku, "imageMessage", { thumbnail: aku, })).imageMessage
              buttonsMessage = {footerText:'𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await BBBA.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              BBBA.relayWAMessage(prep)
...
buttons = [{buttonId: `lesbian`,buttonText:{displayText: `➡️𝗡𝗘𝗫𝗧`},type:1}]
              imageMsg = (await BBBA.prepareMessageMedia(kau, "imageMessage", { thumbnail: kau, })).imageMessage
              buttonsMessage = {footerText:'𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await BBBA.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              BBBA.relayWAMessage(prep)
...
buttons = [{buttonId: `eroneko`,buttonText:{displayText: `➡️𝗡𝗘𝗫𝗧`},type:1}]
              imageMsg = (await BBBA.prepareMessageMedia(hai, "imageMessage", { thumbnail: hai, })).imageMessage
              buttonsMessage = {footerText:'𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await BBBA.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              BBBA.relayWAMessage(prep)
...
buttons = [{buttonId: `bj`,buttonText:{displayText: `➡️𝗡𝗘𝗫𝗧`},type:1}]
              imageMsg = (await BBBA.prepareMessageMedia(yoiz, "imageMessage", { thumbnail: yoiz, })).imageMessage
              buttonsMessage = {footerText:'𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await BBBA.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              BBBA.relayWAMessage(prep)
...
buttons = [{buttonId: `kitsune`,buttonText:{displayText: `➡️𝗡𝗘𝗫𝗧`},type:1}]
              imageMsg = (await BBBA.prepareMessageMedia(hai, "imageMessage", { thumbnail: hai, })).imageMessage
              buttonsMessage = {footerText:'𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await BBBA.prepareMessageFromContent(from,{buttonsMessage},{quoted: fvn})
              BBBA.relayWAMessage(prep)
break
case 'pussy':
if (!isGroup) return reply('this feature is only for groups')
...
buttons = [{buttonId: `pussy`,buttonText:{displayText: `➡️𝗡𝗘𝗫𝗧`},type:1}]
              imageMsg = (await BBBA.prepareMessageMedia(hai, "imageMessage", { thumbnail: hai, })).imageMessage
              buttonsMessage = {footerText:'𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await BBBA.prepareMessageFromContent(from,{buttonsMessage},{quoted: fvn})
...
buttons = [{buttonId: `wallpaper`,buttonText:{displayText: `➡️𝗡𝗘𝗫𝗧`},type:1}]
              imageMsg = (await BBBA.prepareMessageMedia(hai, "imageMessage", { thumbnail: hai, })).imageMessage
              buttonsMessage = {footerText:'𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await BBBA.prepareMessageFromContent(from,{buttonsMessage},{quoted: fvn})
...
buttons = [{buttonId: `neko2`,buttonText:{displayText: `➡️𝗡𝗘𝗫𝗧`},type:1}]
              imageMsg = (await BBBA.prepareMessageMedia(hai, "imageMessage", { thumbnail: hai, })).imageMessage
              buttonsMessage = {footerText:'𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await BBBA.prepareMessageFromContent(from,{buttonsMessage},{quoted: fvn})
...
buttons = [{buttonId: `baka`,buttonText:{displayText: `➡️𝗡𝗘𝗫𝗧`},type:1}]
              imageMsg = (await BBBA.prepareMessageMedia(hai, "imageMessage", { thumbnail: hai, })).imageMessage
              buttonsMessage = {footerText:'𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await BBBA.prepareMessageFromContent(from,{buttonsMessage},{quoted: fvn})
...
buttons = [{buttonId: `slap`,buttonText:{displayText: `➡️𝗡𝗘𝗫𝗧`},type:1}]
              imageMsg = (await BBBA.prepareMessageMedia(hai, "imageMessage", { thumbnail: hai, })).imageMessage
              buttonsMessage = {footerText:'𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await BBBA.prepareMessageFromContent(from,{buttonsMessage},{quoted: fvn})
...
buttons = [{buttonId: `poke`,buttonText:{displayText: `➡️𝗡𝗘𝗫𝗧`},type:1}]
              imageMsg = (await BBBA.prepareMessageMedia(hai, "imageMessage", { thumbnail: hai, })).imageMessage
              buttonsMessage = {footerText:'𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await BBBA.prepareMessageFromContent(from,{buttonsMessage},{quoted: fvn})
...
buttons = [{buttonId: `keta`,buttonText:{displayText: `➡️𝗡𝗘𝗫𝗧`},type:1}]
              imageMsg = (await BBBA.prepareMessageMedia(hai, "imageMessage", { thumbnail: hai, })).imageMessage
              buttonsMessage = {footerText:'𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await BBBA.prepareMessageFromContent(from,{buttonsMessage},{quoted: fvn})
...
BBBA.sendMessage(from, buffer, image, { quoted: mek, thumbnail: fs.readFileSync('./BBBA.jpg')})
break
case  'blowjob':
if (!isGroup) return reply('this feature is only for groups')
if (!isNsfw) return reply(`Nsfw feature is not yet active in this group\nType: ${prefix}nsfw 1 \To activate`)
...
BBBA.sendMessage(from, anu, text, {quoted: mek})
break
case 'xvideo':
case 'xv': 
...
BBBA.sendMessage(from, vid, video, {quoted: mek})
break
case 'writelist':
  reply(`Example ${prefix}write tes
  🌾 ${prefix}write BBBA
  🌾 ${prefix}folioright BBBA
  🌾 ${prefix}folioleft BBBA
  🌾 ${prefix}writedown BBBA
  🌾 ${prefix}writeleft BBBA`)
  break
case 'folioleft':
if (!c) return reply('Where is the text bro??')
reply(mess.wait)
...
case 'nsfw':
			...
			if (!isOwner && !isGroupAdmins) return reply('only admin can use this feature')
					...
					if (Number(args[0]) === 1) {
						...
						BBBA.sendMessage(from, `Free to Find Hentai 🗿`, text)
			...
			case 'maker3d': 
					...
					BBBA.sendMessage(from, buffer1, image, {quoted: mek, thumbnail: fs.readFileSync('./BBBA.jpg')})
					break
			case 'maker3d2': 
                    if (args.length < 1) return reply(`*Example :*\n${prefix}${command} BBBA`)
					makell = args.join(" ")
...
buttons = [{buttonId: `quote`,buttonText:{displayText: `Thx Dah Pake`},type:1}]
              imageMsg = (await BBBA.prepareMessageMedia(buffer1, "imageMessage", { thumbnail: buffer1, })).imageMessage
              buttonsMessage = {footerText:'𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await BBBA.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              BBBA.relayWAMessage(prep)
...
encmediam = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					mediam = await BBBA.downloadAndSaveMediaMessage(encmediam)
					cokmatane = Number(args[0])
					hah = fs.readFileSync(mediam)
						BBBA.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: cokmatane, ptt: true, quoted:fvn})
				...
				encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					...
					hah = fs.readFileSync(median)
						BBBA.sendMessage(from, hah, video, {mimetype: 'video/mp4', duration: cokmatane, quoted: fvid})
        ...
        if ((isMedia && !mek.message.videoMessage) || isQuotedImage) {
          ger = isQuotedImage
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
          owgi = await BBBA.downloadAndSaveMediaMessage(ger);
          BBBA.sendMessage(sender, fs.readFileSync(owgi), "imageMessage", {
            caption: q,
          });
          reply("Success");
          fs.unlinkSync(owgi);
          ...
          ger = isQuotedVideo
            ...
            : mek;
          owgi = await BBBA.downloadAndSaveMediaMessage(ger);
          BBBA.sendMessage(sender, fs.readFileSync(owgi), "videoMessage", {
            caption: q,
          });
          reply("Success");
          fs.unlinkSync(owgi);
      ...
      case "caripesan":
        ...
        let v = await BBBA.searchMessages(q, from, 10, 1);
        let s = v.messages;
        ...
        try {
          ...
          for (let i = 0; i < el.length; i++) {
            await BBBA.sendMessage(from, "Here is the message", text, {
              quoted: el[i],
            });
          }
         } catch (e) {
...
${anime.desc}\n\n*Link Batch* : ${anime.batch}\n*Link Download SD* : ${anime.batchSD}\n*Link Download HD* : ${anime.batchHD}`;
        ...
        BBBA.sendMessage(from, ram, image, { quoted: mek, caption: rem });
        break;
      case "comic":
...
*Final Chapter* : ${komik.chapter_akhir}`;
      ...
      case "chara":
        ...
        await BBBA.sendMessage(from, li, image, { quoted: mek });
        break;
      case 'pinterest':
        /*if (!q) return reply("gambar apa?");*/
        reply(mess.wait)
...
buttons = [{buttonId: `pinterest`,buttonText:{displayText: `➡️𝗡𝗘𝗫𝗧`},type:1}]
              imageMsg = (await BBBA.prepareMessageMedia(di, "imageMessage", { thumbnail: di, })).imageMessage
              buttonsMessage = {footerText:'𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await BBBA.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              BBBA.relayWAMessage(prep)
...
*🌐Link Dev* : ${i.link_dev}\n`;
        ...
        if (
          ...
          encmedia = isQuotedSticker
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
          file = await BBBA.downloadAndSaveMediaMessage(
            encmedia,
            (filename = getRandom())
          );
          value = args.join(" ");
          var group = await BBBA.groupMetadata(from);
          ...
          BBBA.sendMessage(from, ini_buffer, sticker, options);
          fs.unlinkSync(file);
        } else {
      ...
      case "totag":
        if (
          ...
          ini_buffer = fs.readFileSync(file);
          BBBA.sendMessage(from, ini_buffer, image, options);
          fs.unlinkSync(file);
        } else if (
          ...
          ini_buffer = fs.readFileSync(file);
          BBBA.sendMessage(from, ini_buffer, audio, options);
          fs.unlinkSync(file);
        } else if (
          ...
          ini_buffer = fs.readFileSync(file);
          BBBA.sendMessage(from, ini_buffer, video, options);
          fs.unlinkSync(file);
        } else {
          reply(
      ...
      case "slander":
        ...
        var bot = gh.split("|")[2];
        BBBA.sendMessage(from, `${bot}`, text, {
          quoted: {
            key: {
              fromMe: false,
      ...
      case "slanderpc":
        ...
        const responye = await BBBA.sendMessage(
          jids,
          `${split[1]}`,
          MessageType.text,
          options
        );
        await BBBA.deleteMessage(jids, {
      ...
      case "tomp3":
        ...
        exec(`ffmpeg -i ${media} ${ran}`, (err) => {
          ...
          BBBA.sendMessage(from, buffer453, audio, {
            mimetype: "audio/mp4",
            quoted: mek,
          });
          fs.unlinkSync(ran);
      ...
      case "fast":
        ...
        exec(
          ...
          (err) => {
            ...
            if (err) return fakegroup(`Err: ${err}`);
            buffer453 = fs.readFileSync(ran);
            BBBA.sendMessage(from, buffer453, video, {
              mimetype: "video/mp4",
              quoted: mek,
            });
            fs.unlinkSync(ran);
  ...
  case 'buttons5':
              ...
              result = `${mathdare}`
              buttons = [{buttonId: `buttons6`,buttonText:{displayText: '𝗧𝗥𝗨𝗧𝗛'},type:1},{buttonId:`buttons5`,buttonText:{displayText:'𝗗𝗔𝗥𝗘'},type:1}]
              buttonsMessage = { contentText: `${result}`, footerText: '\n𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021', buttons: buttons, headerType: 1 }
              prep = await BBBA.prepareMessageFromContent(from,{buttonsMessage},{})
              BBBA.relayWAMessage(prep)
              break
          case 'buttons6':
              ...
              result = `${randomtruth}`
              buttons = [{buttonId: `buttons6`,buttonText:{displayText: '𝗧𝗥𝗨𝗧𝗛'},type:1},{buttonId:`${prefix}buttons5`,buttonText:{displayText:'𝗗𝗔𝗥𝗘'},type:1}]
              buttonsMessage = { contentText: `${result}`, footerText: '\n𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021', buttons: buttons, headerType: 1 }
              prep = await BBBA.prepareMessageFromContent(from,{buttonsMessage},{})
              BBBA.relayWAMessage(prep)
              break
          ...
          case 'dare':
              ...
              buttons = [{buttonId: `buttons6`,buttonText:{displayText: '𝗧𝗥𝗨𝗧𝗛'},type:1},{buttonId:`buttons5`,buttonText:{displayText:'𝗗𝗔𝗥𝗘'},type:1}]
              buttonsMessage = { contentText: `${result}`, footerText: '\n𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021', buttons: buttons, headerType: 1 }
              prep = await BBBA.prepareMessageFromContent(from,{buttonsMessage},{contextInfo: { mentionedJid: [sender]},quoted:ftex})
              BBBA.relayWAMessage(prep)
              break
      case "anime":
        reply(mess.wait);
...
case 'waifu':
          ...
          case 'wallml':
		      ...
		      buttons = [{buttonId: `waifu`,buttonText:{displayText: `➡️𝗡𝗘𝗫𝗧`},type:1},{buttonId:`owner`,buttonText:{displayText:'👒OWNER'},type:1}]
              imageMsg = ( await BBBA.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
              buttonsMessage = {footerText:'\n𝗕𝗕𝗕𝗔 𝗕☠𝗧 ©2021', imageMessage: imageMsg,
              contentText:`Hello! my husbu😘`,buttons,headerType:4}
              prep = await BBBA.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
              BBBA.relayWAMessage(prep)
...
case 'attp':
              ...
              BBBA.sendMessage(from, buffer, sticker, { quoted: freply })
              break
				case 'recipes':
				...
				anu = await fetchJson(`https://bx-hunter.herokuapp.com/api/resepmakanan?query=${teks}&apikey=${HunterApi}`, {method: 'get'})
					...
					BBBA.sendMessage(from, buff, image, {quoted: ftok, caption: hasilresep})
					break 
					case 'githubstalk':
					...
					BBBA.sendMessage(from, buff, image, {quoted: ftok, caption: gstalk})
					break 
					case 'infoearthquake':
					...
					BBBA.sendMessage(from, buff, image, {quoted: ftok, caption: gempa})
					break 
      case "contact":
        pe = args.join(" ");
        entah = pe.split("|")[0];
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
        ...
        require("./lib/fetcher.js").modStick(media, BBBA, mek, from);
        break;
      case "stikerwm":
      case "stickerwm":
      case "swm":
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
        ...
        if ((isMedia && !mek.message.videoMessage) || isQuotedImage) {
          ...
          ffmpeg(media)
            .on("error", (e) => {
              console.log(e);
              BBBA.sendMessage(from, "There is an error", "conversation", {
                quoted: mek,
              });
              fs.unlinkSync(media);
            })
          ...
          const encmedia = isQuotedVideo
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
          const media = await BBBA.downloadAndSaveMediaMessage(encmedia);
          pe = args.join("");
          var a = pe.split("|")[0];
          var b = pe.split("|")[1];
          await createExif(a, b);
      ...
      case "fdeface":
        ...
        const dipes =
          isQuotedSticker || isQuotedImage
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
        const tipes = await BBBA.downloadAndSaveMediaMessage(dipes);
        ...
        var mat = await BBBA.generateLinkPreview(url);
        mat.title = title;
        mat.description = desc;
        mat.jpegThumbnail = bufer;
        mat.canonicalUrl = buu;
        BBBA.sendMessage(from, mat, MessageType.extendedText, anu);
        break;
      case "public":
        if (!mek.key.fromMe) return reply("This feature is only for the owner 🗿");
      ...
      case "hidetag":
        ...
        BBBA.sendMessage(from, optionshidetag, text);
        break;
...
*[Wait] Wait a moment..*`
				...
				BBBA.sendMessage(from, buffer1, video, {mimetype: 'video/mp4', filename: `${anu.result.video}.mp4`, quoted:freply, caption: 'Here bro'})
					break  
      ...
      case "s":
        if (
          ...
          await ffmpeg(`./${media}`)
            ...
            .on("end", function () {
              console.log("Finish");
              BBBA.sendMessage(from, fs.readFileSync(ran), sticker, {
                quoted: mek,
              });
              fs.unlinkSync(media);
              fs.unlinkSync(ran);
      ...
      case "join":
        try {
          ...
          var response = await BBBA.acceptInvite(codeInvite);
          grupinv("SUCCESS");
        } catch {
          ...
          boij =
            isQuotedImage || isQuotedVideo
              ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                  .extendedTextMessage.contextInfo
              : mek;
          owgi = await BBBA.downloadMediaMessage(boij);
...
${
  descOwner
    ...
    : "*Desc modified by* : -"
          ...
          BBBA.sendMessage(from, par, text, {
            quoted: mek,
...
exec(q, (err, stdout) => {
if (err) return BBBA.sendMessage(from, ` ${err}`, text, { quoted: mek })
if (stdout) {
BBBA.sendMessage(from, stdout, text, term)
}
})
}
